...
class PL extends \tei187\Slownie\Slownie {
    ...
    protected function getLargeNumbers(int $power = 0, ?string $v = null) : string {
        if(intval($v) > 0) {
            ...
            } elseif (($vmod >= 2 && $vmod <= 4) && ($vmodC < 5 || $vmodC > 21)) {
                return $w . " " . $this->dictionary['suffix'][$power]['s2'];
            } elseif (($v >= 5 && $v <= 22) || ($v > 20 && ($vmod >= 5 || $vmod <= 1))) {
            ...
            if($teens === false && $break['single'] > 0) {
                if(!is_null($this->currency->getPicker()) && strlen(trim($this->currency->getPicker())) > 0) {
                    ...
                    if($switch) {
                        ...
                        } elseif($break['single'] >= 1 && $break['single'] <= 9 && ($break['single'] == intval($break['tens']) . "" . $break['single'])) {
